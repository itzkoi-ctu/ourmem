...
const SearchPage = () => {
  ...
  const { data: results, refetch, isFetching, isError } = useQuery<Session[]>({
    queryKey: ['sessions', 'search', filters],
    queryFn: async () => {
      const res = await apiClient.get('/sessions/search', { params: filters });
  ...
  const handleSearch = (e: React.FormEvent) => {
    e.preventDefault();
    setFilters({ location: location.trim(), moodTag: moodTag.trim(), startDate, endDate });
  };
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      {/* Filter Form */}
      <form onSubmit={handleSearch} className="glassmorphism rounded-3xl p-6 border border-couple-100/50 shadow-sm grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 items-end">
        <div className="flex flex-col gap-1.5">
          <label className="text-[10px] font-extrabold uppercase tracking-wider text-stone-500 dark:text-stone-400">
            Location
          </label>
        ...
        <div className="flex flex-col gap-1.5">
          <label htmlFor="end-date" className="muted text-xs font-bold uppercase tracking-wider">End date</label>
          <input id="end-date" type="date" min={startDate || undefined} value={endDate} onChange={e => setEndDate(e.target.value)}
            className="w-full px-4 py-2.5 rounded-xl bg-white dark:bg-stone-900 border border-stone-200 dark:border-stone-700 text-sm" />
        </div>
        <button type="button" className="secondary-button" onClick={() => { setLocation(''); setMoodTag(''); setStartDate(''); setEndDate(''); setFilters(null); }}>Reset filters</button>
        <button
          disabled={isFetching}
          type="submit"
        ...
        >
          ...
          <span>{isFetching ? 'Searching…' : 'Apply filters'}</span>
        </button>
      </form>

      {/* Results grid */}
      <div className="flex flex-col gap-4 mt-4">
        {isError ? <QueryError onRetry={() => refetch()} /> : isFetching ? (
          ...
          results.length > 0 ? (
            ...
            <div className="text-center py-16 bg-white dark:bg-stone-900/40 rounded-3xl border border-stone-100 dark:border-stone-800 flex flex-col items-center justify-center">
              <span className="text-stone-500 font-semibold dark:text-stone-400">No memories match your query.</span>
            </div>
          )
        ) : (
          <div className="text-center py-16 bg-white dark:bg-stone-900/40 rounded-3xl border border-stone-100 dark:border-stone-800 flex flex-col items-center justify-center text-stone-500 dark:text-stone-400">
